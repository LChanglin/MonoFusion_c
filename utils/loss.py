...
def get_loss_multidd(scale_preds, disp_preds, disp_preds2, flow_preds, dc_gt, disp_gt, disp_gt2, flow_gt, valid, epoch, gamma=0.9):

    n_predictions_s = len(scale_preds)
    n_predictions_d = len(disp_preds)
    ...
    for i in range(n_predictions_s):
        ...
        if i<n_predictions_d:
            disp = disp_preds[i]
            disp_nan = ~torch.isnan(disp) & ~torch.isnan(disp_gt)
            valid_d = valid[:, None] & disp_nan
            # disp_gt[valid_d] = 1e8
            mask_disp = maskdc & valid_d
            d_loss = (disp-disp_gt).abs()
            dloss = (mask_disp * d_loss).sum()/(mask_disp.sum() + 1e-8)

            disp2 = disp_preds2[i]
            d_loss2 = (disp2 - disp_gt2).abs()
            dloss2 = (mask_disp * d_loss2).sum()/(mask_disp.sum() + 1e-8)
            
            dloss_f = dloss
            disp_loss += i_weight * (dloss + dloss2)
    ...
    loss = alpha_s*scale_loss
    if torch.isnan(f_loss).sum()<=0:
        loss += alpha_f*flow_loss
    if torch.isnan(disp_loss).sum()<=0:
        loss += alpha_d*disp_loss
    ...
    # print(torch.mean(disp), torch.mean(disp_gt))
    # print(torch.mean(scale))
    # print(disp.shape, disp_gt.shape, flow.shape, flow_gt.shape)
    eped = torch.sum(torch.abs(disp - disp_gt), dim=1)
    eped = epe.view(-1)
    # magd = (disp_gt**2).sqrt()
    # magd = magd.view(-1)
    # outd = ((eped > 3.0) & ((eped/magd) > 0.05)).float()
    # d1 = torch.mean(outd[val])

    d1 = torch.mean(eped[val])

    return sloss_f, loss, dloss_f, gt_dchange, f1, d1
