...
def viz_feature(feature1):
    feature1 = feature1.squeeze(0)  # [c, h, w]

    max_vals, _ = torch.max(feature1.view(feature1.size(0), -1), dim=1)
    _, top3_indices = torch.topk(max_vals, 3)
    top3_channels = feature1[top3_indices]  # [3, h, w]
    top3_channels = (top3_channels - top3_channels.min()) / (top3_channels.max() - top3_channels.min())
